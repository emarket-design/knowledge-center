...
$container = apply_filters('emd_change_container','emd-wrapper','knowledge_center', 'single'); 
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'knowledge_center', 'single');
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
		...
		?>
<?php 	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'knowledge_center', 'single');
	if($has_navigation){
		global $wp_query;
...
<?php 	}
	...
	$show_edit_link = apply_filters('emd_show_single_edit_link',true,'knowledge_center');
	if($show_edit_link){
